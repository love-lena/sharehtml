...
banner "ShareHTML on artifacts.lena.dog"

stage "Confirm the Cloudflare zone"
say "The custom hostname must belong to an active Cloudflare-managed zone."
open_url "https://dash.cloudflare.com/"
step "Open lena.dog and confirm its status is Active."
step "Open DNS > Records and confirm there is no existing record named artifacts."
note "Wrangler will create the DNS record and TLS certificate; do not add a CNAME."
pause "Press Enter after the zone and hostname are ready."

stage "Enable R2 storage"
say "ShareHTML stores uploaded HTML in a private R2 bucket named sharehtml-documents."
open_url "https://dash.cloudflare.com/?to=/:account/r2/overview"
step "Open Storage & databases > R2 object storage and choose Enable or Purchase."
...
step "Open Integrations > Identity providers, then select Add new identity provider."
step "Choose One-time PIN, or configure your preferred identity provider."
...
if ! npx wrangler whoami 2>/dev/null | grep -q "Account ID"; then
  step "A browser login will open now. Approve the account that owns lena.dog."
  npx wrangler login
fi
npx wrangler whoami
